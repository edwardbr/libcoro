...
{
    REQUIRE(sizeof(coro::detail::promise<void>) >= sizeof(std::coroutine_handle<>) + sizeof(rpc::exception_ptr));
    REQUIRE(
        sizeof(coro::detail::promise<int32_t>) ==
        sizeof(std::coroutine_handle<>) + sizeof(std::variant<int32_t, rpc::exception_ptr>));
    REQUIRE(
        sizeof(coro::detail::promise<int64_t>) >=
        sizeof(std::coroutine_handle<>) + sizeof(std::variant<int64_t, rpc::exception_ptr>));
    REQUIRE(
        sizeof(coro::detail::promise<std::vector<int64_t>>) >=
        sizeof(std::coroutine_handle<>) + sizeof(std::variant<std::vector<int64_t>, rpc::exception_ptr>));
}
